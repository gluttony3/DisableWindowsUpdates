...
class WindowsUpdatesEnabler:
    ...
    def enable_update_registry(self) -> bool:
        ...
        # Видаляємо ключ обмеження доступу якщо він є
        try:
            key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate", 0, winreg.KEY_SET_VALUE)
            winreg.DeleteValue(key, "DisableWindowsUpdateAccess")
            winreg.CloseKey(key)
            print("✅ Видалено DisableWindowsUpdateAccess")
        except FileNotFoundError:
            pass
        except Exception as e:
            print(f"⚠️  Не вдалось видалити DisableWindowsUpdateAccess: {e}")
